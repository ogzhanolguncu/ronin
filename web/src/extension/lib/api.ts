...
async function request<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(response.status, errBody?.error ?? `HTTP ${response.status}`, errBody?.code)
  }
...
export const api = {
  ...
  getMetadata: (url: string) =>
    request<MetadataResponse>(`/api/v1/metadata?url=${encodeURIComponent(url)}`),
  ...
  getBookmarkByUrl: async (url: string): Promise<Bookmark | null> => {
    try {
      return await request<Bookmark>(`/api/v1/bookmarks/exists?url=${encodeURIComponent(url)}`)
    } catch (err) {
