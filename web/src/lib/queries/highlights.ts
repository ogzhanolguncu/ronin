...
export function readableContentQueryOptions(bookmarkId: number) {
  return queryOptions({
    ...
    queryFn: () =>
      requester<ReadableContent>(
        `/api/v1/assets/${bookmarkId}/readable/content`,
      ),
    staleTime: Infinity,
  });
}
...
export function highlightsQueryOptions(bookmarkId: number) {
  return queryOptions({
    ...
    queryFn: () =>
      requester<HighlightsResponse>(
        `/api/v1/bookmarks/${bookmarkId}/highlights`,
      ).then((r) => r.highlights),
  });
}
  ...
  return useMutation({
    ...
    onSettled: (_d, _e, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["highlights", variables.bookmark_id],
      });
    },
  });
}
